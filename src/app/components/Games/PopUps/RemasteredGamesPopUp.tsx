...
import he from "he";
import { rgTitleMap } from "@/lib/rgGameTitles";
import { RGGamesProps } from "@/lib/types";

export const RemasteredGamesPopUp: React.FC<RGGamesProps> = ({ isOpen, onClose, title, release_date_JP, release_date_NA, platform, description, src }) => {
    ...
    if (he.decode(title) === he.decode(rgTitleMap[0])) {
    ...
    // Kingdom Hearts ReCOM, Kingdom Hearts Re:Coded HD
    if (he.decode(title) === he.decode(rgTitleMap[1]) || he.decode(title) === he.decode(rgTitleMap[5])) {
    ...
    if (he.decode(title) === he.decode(rgTitleMap[2])) {
    ...
    if (he.decode(title) === he.decode(rgTitleMap[3])) {
    ...
    if (he.decode(title) === he.decode(rgTitleMap[4])) {
        popUpContainer += " bg-[linear-gradient(60deg,rgba(0,229,255,1)0%,rgba(184,234,255,1)100%)]";
        closeButton = closeButtonKH2;
    }

    // DDD
    if (he.decode(title) === he.decode(rgTitleMap[6])) {
